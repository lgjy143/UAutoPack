...
function BuildSln
{
   param([string]$slns, [string]$packagetempdir)
   $build=GetMsBuildPath -Use32BitMsBuild:$true
   $toolsVersion=GetVisualStudioToolsVersion
   Write-Host "MsBuild Path: $build`n"
   ##/p:VisualStudioVersion=12.0
   ##/p:_PackageTempDir="C:\Users\U724\Desktop\dev\artifact"
   $result = ."$build" $slns $toolsVersion /t:Rebuild /p:Configuration=Release /p:DeployOnBuild=true /p:DeployTarget=PipelinePreDeployCopyAllFilesToOneFolder /p:AutoParameterizationWebConfigConnectionStrings=false /p:_PackageTempDir=$packagetempdir
   return $result
}	
